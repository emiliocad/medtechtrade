...
class Tsk_Image_ImageMagick
    ...
    protected function _resize( $newFile , $newWidth , $newHeight )
        ...
        $imagick->resizeImage( $newWidth , $newHeight ,
    ...
    protected function _crop( $newFile , $resizeWidth , $resizeHeight ,
                              $newWidth , $newHeight , $cropX , $cropY ,
                              $resize = true )
        {
        $imagick = new Imagick();
        $imagick->readImage( $this->_file );
    ...
    public function watermarkImage( $overlayFile , $type )
        ...
        foreach ( $positions as $position )
            {
            $colors = $imagick->getImageRegion( $overlayWidth , $overlayHeight ,
                                                $position[0] , $position[1] )->getImageColors();
        ...
        $imagick->compositeImage( $overlay , Imagick::COMPOSITE_OVER , $min[0] ,
                                  $min[1] );
    ...
    public function watermarkText( $overlayText , $position ,
        ...
        switch ( $position )
            ...
            default:
                throw new Exception(
                        'Do not support ' . $position . ' type of alignment'
                );
                break;
            }

        $draw->setFillColor( '#' . $param['color'] );
        $imagick->annotateImage( $draw , 5 , 5 , $param['rotation'] ,
                                 $overlayText );
